...
(function () {
  ...
  // --- Cookie consent banner (localized by page language) ---
  var cookieConsentKey = 'drcenik-cookie-consent';
  if (!localStorage.getItem(cookieConsentKey)) {
    var cookieLang = (document.documentElement.getAttribute('lang') || 'de').split('-')[0];
    var cookieI18n = {
      de: {
        aria: 'Cookie-Hinweis',
        text: 'Wir verwenden Cookies, um die Nutzung der Website zu erleichtern.',
      ...
      en: {
        aria: 'Cookie notice',
        text: 'We use cookies to improve your experience on this website.',
        linkLabel: 'Privacy policy',
        linkHref: 'datenschutz-en.html',
        necessary: 'Necessary only',
        accept: 'Accept all'
      },
      tr: {
        aria: 'Cerez bildirimi',
        text: 'Bu web sitesinin kullanimini iyilestirmek icin cerezler kullaniyoruz.',
        linkLabel: 'Gizlilik politikasi',
        linkHref: 'datenschutz-tr.html',
        necessary: 'Sadece gerekli',
        accept: 'Tumunu kabul et'
      }
    };
    var C = cookieI18n[cookieLang] || cookieI18n.de;
    ...
    banner.innerHTML =
      '<div class="cookie-consent-inner">' +
        '<p class="cookie-consent-text">' + C.text + ' <a href="' + C.linkHref + '">' + C.linkLabel + '</a></p>' +
        '<div class="cookie-consent-actions">' +
          '<button type="button" class="btn btn-secondary cookie-consent-necessary">' + C.necessary + '</button>' +
          '<button type="button" class="btn btn-primary cookie-consent-accept">' + C.accept + '</button>' +
        '</div>' +
      '</div>';
